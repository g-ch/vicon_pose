...
void CFetchViconData::GetStatus(ObjStatus &s, const char * model, const char * segment)
{
   ...
   /* printf("Get a frame...");*/
    ...
    {
        s.res=false;
        // std::cout << "Output_quat False" << std::endl;
        // return s;
        return;
    }
    for(int i=0;i<4;i++)
...
//         std::cout << "quat" << i << "=" <<s.ort[i] << std::endl;
    ...
    if(Output_euler.Result!=Result::Success)
    {
        s.res=false;
